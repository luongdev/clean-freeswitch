...
SOCKETS_RN=$(sed -n '/<list name="socket_acl" default=".*">/=' /usr/local/freeswitch/conf/autoload_configs/acl.conf.xml | head -n 1)
...
add_esl() {
    ...
    sed -i "${SOCKETS_RN}a\ \t<node type=\"allow\" cidr=\"$cidr\"/>" /usr/local/freeswitch/conf/autoload_configs/acl.conf.xml
